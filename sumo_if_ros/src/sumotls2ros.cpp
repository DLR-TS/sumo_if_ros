...
void SumoTLs2Ros::getGeoreferencedLinks(
    std::string sumo_intersection_id,
...
{
    auto links = libsumo::TrafficLight::getControlledLinks(sumo_intersection_id);
    ...
    for (auto&& l : links)
    {
        ...
        for (int i = 0; i < l.size(); i++)
        {
            ...
            if (ingressing_lane.size() != 0 && id_2_shape.find(ingressing_lane) == id_2_shape.end())
            {
                auto pos_vector_ingress = libsumo::Lane::getShape(ingressing_lane);
                id_2_shape.insert(std::make_pair(ingressing_lane, getNodesFromSUMOLane(pos_vector_ingress)));
            }
            if (egressing_lane.size() != 0 && id_2_shape.find(egressing_lane) == id_2_shape.end())
            {
                auto pos_vector_egress = libsumo::Lane::getShape(egressing_lane);
                id_2_shape.insert(std::make_pair(egressing_lane, getNodesFromSUMOLane(pos_vector_egress)));
            }
            if (via_Lane.size() != 0 && id_2_shape.find(via_Lane) == id_2_shape.end())
            {
                auto pos_vector_via = libsumo::Lane::getShape(via_Lane);
                id_2_shape.insert(std::make_pair(via_Lane, getNodesFromSUMOLane(pos_vector_via)));
            }

            if(ingressing_lane.size() == 0 || egressing_lane.size() == 0)
                continue;
...
std::unordered_map<int, v2xsim::SimMAPEM> SumoTLs2Ros::getMAPEMFromSUMO(double time,double power)
{
    ...
    {
        intersections_ = generateMAPEMFromSUMO();
    ...
    std::unordered_map<std::string, adore::sumo_if_ros::MAPEMIntersection> mapem_data, double time, double power)
{
    ...
    for (auto&& item : mapem_data)
    {
        ...
        mapem_ros_msgs[getIntersectionIDForSUMOString(item.first)] = ros_mapem;
...
{
    std::unordered_map<std::string, adore::sumo_if_ros::MAPEMIntersection> intersections;
    ...
    for (auto&& i : tl_id_list)
    {
        ...
        adore::sumo_if_ros::MAPEMIntersection intersection;
        ...
        getGeoreferencedLinks(i, link_list, id_2_shape);

        intersection.intersection_id_ = getIntersectionIDForSUMOString(i);

        intersection.lane_width_ = libsumo::Lane::getWidth((*link_list.begin()).first);

        std::vector<std::string> lane_set;

        auto lanes = libsumo::TrafficLight::getControlledLanes(i);
        ...
        for (auto&& lane : lanes)
        {
            ...
            adore::sumo_if_ros::MAPEMLane mapem_lane;
            ...
            mapem_lane.lanetype_attribute_ = adore::sumo_if_ros::MAPEMLane::VEHICLE;
            ...
            mapem_lane.directional_use_ = adore::sumo_if_ros::MAPEMLane::INGRESS;
            ...
            std::vector<adore::sumo_if_ros::MAPEMLane::ConnectingLane> connectors;

            for (int i = 0; i < linked_lanes.size(); i++)
            {
                auto lane_id = linked_lanes[i];

                adore::sumo_if_ros::MAPEMLane::ConnectingLane cl;
                ...
                cl.allowed_maneuvers_on_connection = adore::sumo_if_ros::MAPEMLane::ConnectingLane::AllowedManeuvers(i + 1);
                ...
                adore::sumo_if_ros::MAPEMLane egressing_lane;

                egressing_lane.lane_id_ = sumo_lane_id_mapping_[lane_id];

                egressing_lane.directional_use_ = adore::sumo_if_ros::MAPEMLane::EGRESS;
        ...
        adore::sumo_if_ros::Coordinate centroid_sum_coord;
        ...
        intersections.insert(std::make_pair(i, intersection));
    }
...
std::vector<adore::sumo_if_ros::Coordinate> SumoTLs2Ros::getNodesFromSUMOLane(libsumo::TraCIPositionVector& lane,
                                                                                   int nodeCount)
{
    std::vector<adore::sumo_if_ros::Coordinate> coordinateSet;

    // TODO change to this after version 1.9 of SUMO is available: for(auto it : lane.value)
    for (auto it = lane.value.begin(); it != lane.value.end(); it++)
    {
        // TODO change to this with SUMO 1.9
        coordinateSet.push_back(adore::sumo_if_ros::Coordinate(it->x, it->y, it->z));
    }
...
std::vector<v2xsim::SimSPATEM> SumoTLs2Ros::getSPATEMFromSUMO(double time, double power)
{
    ...
    for (auto&& i : tl_id_list)
    {
        auto complete_program = libsumo::TrafficLight::getCompleteRedYellowGreenDefinition(i);
        auto program = libsumo::TrafficLight::getProgram(i);
        ...
        spatem.meta.location.x = intersections_[i].ref_longitude_;
        spatem.meta.location.y = intersections_[i].ref_latitude_;
        ...
        auto links = libsumo::TrafficLight::getControlledLinks(i);
